...
def collect_inputs():
    ...
    irb_mask = input("Subnet mask in bits for the BVI or IRB PE interface: ")
    irb_ipv6addr = input("IPv6 address for the BVI or IRB PE interface: ")
    irb_ipv6mask = input("IPv6 subnet mask in bits for the BVI or IRB PE interface: ")
    pe_device = input("Provider Edge Device Hostname: ")
    service_type = input("Service Type (p2p or p2mp): ")
    return (customer_name, access_device, access_interface, circuit_id, qos_input, qos_output, vlan_id, vlan_id_outer, pw_id, irb_ipaddr, irb_mask, irb_ipv6addr, irb_ipv6mask, pe_device, service_type)
...
def valid_ip(address):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(f"Invalid IPv4/6 address: {address}")
...
def main():
    ...
    parser.add_argument("--irb-ipaddr", type=valid_ip, help="IP address for the BVI or IRB PE interface.")
    parser.add_argument("--irb-mask", type=int, help="Subnet mask in bits for the BVI or IRB PE interface.")
    parser.add_argument("--irb-ipv6addr", type=valid_ip, help="IPv6 address for the BVI or IRB PE interface.")
    parser.add_argument("--irb-ipv6mask", type=int, help="IPv6 subnet mask in bits for the BVI or IRB PE interface.")
    ...
    if args.interactive:
        args.customer_name, args.access_device, args.access_interface, args.circuit_id, args.qos_input, args.qos_output, args.vlan_id, args.vlan_id_outer, args.pw_id, args.irb_ipaddr, args.irb_mask, args.irb_ipv6addr, args.irb_ipv6mask, args.pe_device, args.service_type = collect_inputs()
    ...
    context = {
        ...
        'irb_mask': args.irb_mask,
        'irb_ipv6addr': args.irb_ipv6addr,
        'irb_ipv6mask': args.irb_ipv6mask,
